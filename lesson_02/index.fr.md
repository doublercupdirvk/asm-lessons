...
Regardons la syntaxe du calcul des addresses mémoires. Cela s'applique à tout type d'addresses mémoires:
...
* disp - Entier (stocké jusqu'à au plus 32 bits). disp est un décalage dans le données.
...
Notez comment dans  ```movu m1, [srcq+2*r1q+3+mmsize]```  l'assembleur pré-calculera le déplacement correct à utiliser. Dans la prochaine leçon, nous vous présenterons une astuce pour éviter d'avoir ```add``` et ```dec``` dans la boucle, en les remplacements par un unique ```add```.
...
Dans l'exercice, vous aurez à charger une constante et en additioner les valeurs à un vecteur **SIMD** dans une boucle.
